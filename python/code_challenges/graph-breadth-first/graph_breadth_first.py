...
# TODO6: BFS,
    def bfs(self, key):
        """Takes a node as an argument and returns the collection of nodes in the order they were visited."""
        self.visited = set()
        self.queue = Queue()
        if key in self.get_nodes():
            self.queue(key)
            key = self.get_neighbors(key)
        while self.queue:
            s = self.queue.dequeue[0]
            self.visited.add(s)
        return self.visited
