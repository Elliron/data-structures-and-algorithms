...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(person => {
    let photoTemplate = $('#template').clone();
    let $h2 = photoTemplate.find('h2');
    $h2.text(person.name);
    let $h3 = photoTemplate.find('h3');
    $h3.text(person.height);
    let $p = photoTemplate.find('p');
...
const wordsToCharList = (arr) => {
  // Solution code here...
  const newArray = [];
  let stringThatThing = arr.split('');
  newArray.push(stringThatThing);
  // }
  return newArray[0];
};
