...
const templatingWithMustache = () => {
  ...
  characters.forEach((character) => {
    ...
    const renderedHTML = Mustache.render(characterTemplate, character);
    templateStrings.push(renderedHTML);
...
const hasChildrenValues = (arr, character) => {
  ...
  let trueVar;
  valuesArr.forEach(gotCharacter => {
    if (gotCharacter.name === character) {
      const newArr = Array.from(gotCharacter.children);
      if (newArr.length === 0) {
        trueVar = false;
      } else {
        trueVar = true;
